...
class AyatListAdapter(private val settings: Settings) : RecyclerView.Adapter<AyatListViewHolder>() {
    ...
    private var onOptionsClick: (view: View, ayatId: Int) -> Unit = {
            _, _ -> Log.w("Warning", "onOptionsClick function is not set to AyatListAdapter" )
    }
    private var onLinkClick: (Int) -> Unit = {
        _ -> Log.w("Warning", "onLinkClick functions is not set to AyatListAdapter")
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): AyatListViewHolder {
        ...
        return AyatListViewHolder(view, onLinkClick)
    }

    override fun getItemCount(): Int = models.size

    override fun onBindViewHolder(holder: AyatListViewHolder, position: Int) {
        holder.populateModel(models[position], settings, onOptionsClick)
    }
}